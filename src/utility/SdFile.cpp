...
//------------------------------------------------------------------------------
/** List directory contents to port->
...
void SdFile::ls(uint8_t flags, uint8_t indent, HardwareSerial *port)
{
  ...
  while ((p = readDirCache()))
  {
    ...
    printDirName(*p, flags & (LS_DATE | LS_SIZE) ? 14 : 0, port);
...
//------------------------------------------------------------------------------
/** %Print the name field of a directory entry in 8.3 format to port->

   \param[in] dir The directory structure containing the name.
   \param[in] width Blank fill name if length is less than \a width.
*/
void SdFile::printDirName(const dir_t &dir, uint8_t width, HardwareSerial *port)
{
  uint8_t w = 0;
  for (uint8_t i = 0; i < 11; i++)
  {
...
//------------------------------------------------------------------------------
/** %Print a directory date field to port->

    Format is yyyy-mm-dd.

   \param[in] fatDate The date field from a directory entry.
*/
void SdFile::printFatDate(uint16_t fatDate, HardwareSerial *port)
{
  port->print(FAT_YEAR(fatDate));
  port->print('-');
  printTwoDigits(FAT_MONTH(fatDate), port);
  port->print('-');
  printTwoDigits(FAT_DAY(fatDate), port);
}
//------------------------------------------------------------------------------
/** %Print a directory time field to port->

   Format is hh:mm:ss.

   \param[in] fatTime The time field from a directory entry.
*/
void SdFile::printFatTime(uint16_t fatTime, HardwareSerial *port)
{
  printTwoDigits(FAT_HOUR(fatTime), port);
  port->print(':');
  printTwoDigits(FAT_MINUTE(fatTime), port);
  port->print(':');
  printTwoDigits(FAT_SECOND(fatTime), port);
}
//------------------------------------------------------------------------------
/** %Print a value as two digits to port->

   \param[in] v Value to be printed, 0 <= \a v <= 99
*/
void SdFile::printTwoDigits(uint8_t v, HardwareSerial *port)
{
  char str[3];
  str[0] = '0' + v / 10;
  str[1] = '0' + v % 10;
